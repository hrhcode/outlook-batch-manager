...
export function ProxyConfigPanel() {
  ...
  const enabledCount = proxies.filter((p) => p.is_enabled).length;
    ...
    <div className="proxy-config-panel">
      <div className="panel">
        <div className="panel-header">
          ...
          <p className="field-hint">
            配置代理池，批量注册时会随机选择代理以降低IP限制风险
            {proxies.length > 0 && (
              <span className="proxy-stats">
                （共 {proxies.length} 个代理，{enabledCount} 个启用）
              </span>
            )}
          </p>
        </div>
        ...
        <div className="proxy-add-form">
          <input
            ...
            placeholder="输入代理地址，如 http://127.0.0.1:7897 或 socks5://127.0.0.1:1080"
            value={newProxyUrl}
          />
          <button
      ...
      <div className="panel panel-muted">
        ...
        <div className="info-content">
          ...
          <ul>
            <li>HTTP 代理：<code>http://host:port</code></li>
            <li>HTTPS 代理：<code>https://host:port</code></li>
            <li>SOCKS5 代理：<code>socks5://host:port</code></li>
            <li>带认证代理：<code>http://username:password@host:port</code></li>
          </ul>
          <p className="field-hint">
            提示：建议配置多个代理以轮换使用，降低被限制的风险。注册时会随机选择一个启用的代理。
          </p>
        </div>
      </div>
    </div>
